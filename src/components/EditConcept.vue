    ...
    <div class="form-card" v-if="selectorState.selectedLangs.length > 0">
      <form @submit.prevent="handleSubmit">
        <div class="form-group">
          ...
          <textarea id="markdownInput" class="markdown-editor" v-model="form.markdown_content" rows="8"
            spellcheck="false" placeholder="Markdown content here..."></textarea>
        </div>

        <div v-for="code in selectorState.selectedLangs" :key="code" class="form-language-block">
          <h4>{{ getLangName(code) }}</h4>
          <input type="text" v-model="form.titles[code]" placeholder="Title" />
        </div>
...
const dynamicTitle = computed(() => {
  for (const code of langPriority) {
    const title = form.titles?.[code]
    if (title?.trim()) return `Editing concept: ${title.trim()}`
  }
  ...
  try {
    const payload = {
      ...
      concept: {
        ...
        titles: selectorState.value.selectedLangs.map(code => ({
          language: code,
          title: form.titles[code] || ''
        }))
      }
    }
...
const resetForm = () => {
  ...
  form.markdown_content = entry.markdown_content || ''
  form.titles = {}

  entry.titles.forEach(t => {
    form.titles[t.language] = t.title || ''
  })
...
watch(() => props.entryData, (newVal) => {
  if (newVal?.type === 'concept') {
    form.markdown_content = newVal.markdown_content || ''
    form.titles = {}
    newVal.titles.forEach(t => {
      form.titles[t.language] = t.title || ''
    })
    selectorState.value.selectedLangs = newVal.titles.map(t => t.language)
