    ...
    <div class="form-card" v-if="selectorState.selectedLangs.length > 0">
      <form @submit.prevent="handleSubmit">
        <div v-for="code in selectorState.selectedLangs" :key="code" class="form-language-block">
          ...
          <textarea v-model="form.term[code].definition" placeholder="Definition" rows="2"
...
const dynamicTitle = computed(() => {
  for (const code of langPriority) {
    const translation = form.term?.[code]?.translation
    if (translation?.trim()) return `Editing term: ${translation.trim()}`
  }
  return 'Editing term'
})
...
const handleSubmit = async () => {
  try {
    const payload = {
      ...
      term: selectorState.value.selectedLangs.map(code => ({
        language: code,
...
const resetForm = () => {
  ...
  Object.keys(form.term).forEach(lang => {
    form.term[lang] = { translation: '', definition: '' };
  });

  // Then refill from original data
  entry.translations.forEach(t => {
    form.term[t.language] = {
      translation: t.translation || '',
      definition: t.definition || ''
    };
  });

  // Leave selectorState.selectedLangs untouched
  // Optionally reset group fields (remove if you want to preserve them too)
  selectorState.value.groupId = entry.group_id || '';
};

watch(() => props.entryData, (newVal) => {
  if (newVal?.type === 'term' && Array.isArray(newVal.translations)) {
    form.term = {}
    newVal.translations.forEach(t => {
      form.term[t.language] = {
        translation: t.translation || '',
        definition: t.definition || ''
      }
    })
    selectorState.value.selectedLangs = newVal.translations.map(t => t.language)
